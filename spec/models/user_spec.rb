  ...
  describe 'is invalid' do
    ...
    context 'with repeated' do
      it 'usernames' do
        ...
        expect(FactoryGirl.build(:user, username: 'creative_username')).to be_invalid
      end

      it 'emails' do
        FactoryGirl.create(:user, email: 'repeated@email.com')
        expect(FactoryGirl.build(:user, email: 'repeated@email.com')).to be_invalid
      end

      it 'phone numbers' do
        FactoryGirl.create(:user, phone_number: 999_999_999)
        expect(FactoryGirl.build(:user, phone_number: 999_999_999)).to be_invalid
      end
    end
  end
